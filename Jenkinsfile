pipeline {
  ...
  stages {
    ...
    stage('LocalStack S3 - create bucket') {
      steps {
        sh '''
          set -euxo pipefail

          # 1) Levantar LocalStack solo con S3
          docker compose -f ci/localstack/docker-compose.local.yml up -d

          # 2) Esperar a que S3 esté "running"
          for i in $(seq 1 60); do
            if curl -fsS http://localhost:4566/_localstack/health | grep -E '"s3"\\s*:\\s*"running"'; then
          ...
          # 3) Instalar awslocal (envoltura de AWS CLI para LocalStack)
          python -m pip install --upgrade pip awscli-local >/dev/null 2>&1 || pip install --upgrade pip awscli-local

          # 4) Exportar variables (credenciales dummy válidas para LocalStack)
      ...
      post {
        always {
          echo 'LocalStack S3: stage finalizado.'
        }
      }
    }
  }
